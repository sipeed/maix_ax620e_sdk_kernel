...
static int __init aicbsp_init(void)
{
	...
	np = of_find_compatible_node(NULL, NULL, "aicsemi,aic_bsp");
	if (!np) {
		pr_err("failed to find aic-bsp node in device tree\n");
		return -ENODEV;
	}
	aicbsp_pdev->dev.of_node = np;
	...
	reset_gpio = devm_gpiod_get(&aicbsp_pdev->dev, "reset", GPIOD_OUT_HIGH);
	if (IS_ERR(reset_gpio)) {
		dev_err(&aicbsp_pdev->dev, "Failed to get reset-gpio\n");
