...
#define dev_err_probe(dev, err, fmt, ...) ({ \
    ...
    if (_err == -EPROBE_DEFER) \
        dev_dbg(dev, "Deferred probe: " fmt, ##__VA_ARGS__); \
    else if (_err != 0) \
        dev_err(dev, "Error %d: " fmt, _err, ##__VA_ARGS__); \
    _err; \
})
...
static int iio_parse_mount_matrix(const struct device *dev,
			  const char *propname,
			  const char *str,
			  struct iio_mount_matrix *matrix)
{
	char *str_copy, *p, *token;
	int i = 0;
	...
	p = str_copy;
	while ((token = strsep(&p, ",;")) && i < 9) {
		while (*token == ' ')
			token++;
		matrix->rotation[i] = kstrdup(token, GFP_KERNEL);
		if (!matrix->rotation[i]) {
			while (i-- > 0)
				kfree(matrix->rotation[i]);
			kfree(str_copy);
			return -ENOMEM;
		}
		i++;
...
/*
 * 兼容版本的 iio_read_mount_matrix
 * 适用于 Linux 4.19.125 等较旧内核
 */
static int iio_read_mount_matrix(struct device *dev,
				struct iio_mount_matrix *matrix)
{
	/* 返回默认单位矩阵 */
	const char *default_matrix = "1, 0, 0; 0, 1, 0; 0, 0, 1";
	return iio_parse_mount_matrix(dev, "mount-matrix", 
