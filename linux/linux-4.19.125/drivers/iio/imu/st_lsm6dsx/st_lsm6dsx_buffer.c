...
int st_lsm6dsx_fifo_setup(struct st_lsm6dsx_hw *hw)
{
	...
	for (i = 0; i < ST_LSM6DSX_ID_MAX; i++) {
		...
		/* Allocate a kfifo buffer */
		buffer = iio_kfifo_allocate();
		if (!buffer) {
			dev_err(hw->dev, "Failed to allocate kfifo buffer for dev %d\n", i);
			return -ENOMEM;
		}

		/* Attach the buffer to the IIO device */
		iio_device_attach_buffer(hw->iio_devs[i], buffer);

		/* Configure buffer mode and operations */
