...
static int backport_power_supply_get_battery_info(struct power_supply *psy,
                                                 struct power_supply_battery_info **info)
{
    *info = kzalloc(sizeof(**info), GFP_KERNEL);
    if (!*info)
        return -ENOMEM;
    return power_supply_get_battery_info(psy, *info);
}
#define power_supply_get_battery_info backport_power_supply_get_battery_info
#endif
...
static int cw2015_parse_properties(struct cw_battery *cw_bat)
{
	...
	length = of_property_count_u8_elems(dev->of_node, "cellwise,battery-profile");
	if (length < 0) {
		dev_warn(cw_bat->dev,
