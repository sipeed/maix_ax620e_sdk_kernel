		...
		*/
		if (snd_soc_component_get_bias_level(codec) == SND_SOC_BIAS_OFF)
		{
			if (!IS_ERR(priv->mclk))
			{
...
#endif
				snd_soc_component_write(codec, ES8156_SCLK_MODE_REG02, 0x00);
				snd_soc_component_write(codec, ES8156_ANALOG_SYS1_REG20, 0x2A);
				snd_soc_component_write(codec, ES8156_ANALOG_SYS2_REG21, 0x3C);
				snd_soc_component_write(codec, ES8156_ANALOG_SYS3_REG22, 0x0A);
				snd_soc_component_write(codec, ES8156_ANALOG_LP_REG24, 0x07);
				snd_soc_component_write(codec, ES8156_ANALOG_SYS4_REG23, 0x00);
				snd_soc_component_write(codec, ES8156_TIME_CONTROL1_REG0A, 0x03);
				snd_soc_component_write(codec, ES8156_TIME_CONTROL2_REG0B, 0x03);
				...
				snd_soc_component_write(codec, ES8156_ANALOG_SYS5_REG25, 0x10);
#if MCLK
				if (ret) {
					dev_err(codec->dev,
...
struct i2c_driver es8156_i2c_driver = {
	.driver = {
		.name		= "es8156",
