...
/*
	case 12000000:
	case 24000000:
		es7243l->sysclk_constraints = &constraints_12;
		es7243l->sysclk = freq;
		return 0;
*/
	}
		      ...
		      struct snd_soc_dai *dai)
{
	...
	if (coeff < 0) {
		printk("Unable to configure sample rate %dHz with %dHz MCLK",
		       params_rate(params), es7243l->sysclk);
		return coeff;
	}
...
static int
es7243l_set_bias_level(struct snd_soc_component *component,
...
#define es7243l_RATES SNDRV_PCM_RATE_8000_48000

#define es7243l_FORMATS (SNDRV_PCM_FMTBIT_S16_LE | SNDRV_PCM_FMTBIT_S20_3LE |\
	SNDRV_PCM_FMTBIT_S24_LE | SNDRV_PCM_FMTBIT_S32_LE)

static struct snd_soc_dai_ops es7243l_ops = {
...
static int es7243l_probe(struct snd_soc_component *component)
{
	...
	printk("begin->>>>>>>>>>%s!\n", __func__);
#if !ES7243l_CODEC_RW_TEST_EN
	//ret = snd_soc_component_set_cache_io(codec, 8, 8, SND_SOC_I2C);//8,8
#else
	component->control_data = devm_regmap_init_i2c(es7243l->i2c,
						   &es7243l_regmap_config);
	...
	for (index = 0; index < (ES7243l_CHANNELS_MAX) / 2; index++) {
		...
		es7243l_write(ES7243l_ADCCTL4,0x15,i2c_clt[index]);
		...
		if(0x72==regv){
			...
			es7243l_write(ES7243l_ADC_BIAS_0x18, 0x24, i2c_clt[index]);
			es7243l_write(ES7243l_VMIDSEL, 0x02, i2c_clt[index]);
			es7243l_write(ES7243l_PGA_BIAS, 0x88, i2c_clt[index]);
			es7243l_write(ES7243l_ADC_BIAS_0x1A, 0x84, i2c_clt[index]);
			es7243l_write(ES7243l_ADC_MICBIAS, 0x66, i2c_clt[index]);
			es7243l_write(ES7243l_ADC_VRPBIAS, 0x44, i2c_clt[index]);
			es7243l_write(ES7243l_ADC_LP, 0xF0, i2c_clt[index]);
		...
		//es7243l_write(ES7243l_ADC_VMID, 0x03, i2c_clt[index]);
		/*
		 * reset PGA
		 */
		msleep(100);
...
static struct snd_soc_component_driver soc_codec_dev_es7243l = {
	.probe = es7243l_probe,
...
#if ES7243l_CODEC_RW_TEST_EN
	.read = es7243l_codec_read,
	.write = es7243l_codec_write,
#endif
...
es7243l_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	printk("eg read star addres=0x06,count 0x10:echo 0610 >es7243l\n");
	printk
...
es7243l_i2c_probe(struct i2c_client *i2c, const struct i2c_device_id *i2c_id)
{
	...
	//i2c_set_clientdata(i2c, es7243l);
	//es7243l->regmap = devm_regmap_init_i2c(i2c, &es7243l_regmap);
...
struct i2c_driver es7243l_i2c_driver = {
	.driver = {
		   .name = "es7243l",
		   .owner = THIS_MODULE,
#if ES7243l_MATCH_DTS_EN
