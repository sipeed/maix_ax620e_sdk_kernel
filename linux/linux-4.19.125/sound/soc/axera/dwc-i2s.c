		      ...
		      struct snd_pcm_substream *substream)
{
	...
	if (dev->use_pio == false) {
		if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
			...
			i2s_write_reg(dev->i2s_base, I2S_DMACR, dmacr | (0x1 << DMAEN_TXBLOCK_SHIFT));
		} else {
			dmacr = i2s_read_reg(dev->i2s_base, I2S_DMACR);
			i2s_write_reg(dev->i2s_base, I2S_DMACR, dmacr | (0x1 << DMAEN_RXBLOCK_SHIFT));
		}
	}
		...
		struct snd_pcm_hw_params *params, struct snd_soc_dai *dai)
{
	...
	// check whether this is a hdmi i2s
	// due to hdmi only support i2sclk = 64 * fs
	if (dev->hdmi_i2s) {
		config->data_width = 32;
		dev->ccr = 0x10;
	}
				   ...
				   unsigned int rates)
{
	...
	if (COMP1_TX_ENABLED(comp1)) {
		...
		dw_i2s_dai->playback.formats = dw_get_formats(idx);
	...
	if (COMP1_RX_ENABLED(comp1)) {
		...
		dw_i2s_dai->capture.formats = dw_get_formats(idx);
...
static const struct of_device_id dw_i2s_of_match[] = {
	{ .compatible = "axera,dwc-i2s-mst", .data = (void *)I2S_MASTER },
	{ .compatible = "axera,dwc-i2s-slv", .data = (void *)I2S_SLAVER },
	{},
};
...
static int dw_i2s_probe(struct platform_device *pdev)
{
	...
	of_id = of_match_device(dw_i2s_of_match, &pdev->dev);
	if (!of_id)
		return -EINVAL;
